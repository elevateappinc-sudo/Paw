...
export function PetMembersPanel({ petId }: Props) {
  ...
  const fetchMembers = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("pet_members")
      .select(`
        id, pet_id, member_id, status, business_id, created_at,
  ...
  async function handleApprove(memberId: string) {
    const supabase = createClient();
    await supabase.from("pet_members").update({ status: "active" }).eq("id", memberId);
    void fetchMembers();
  }

  async function handleRevoke(memberId: string) {
    const supabase = createClient();
    await supabase.from("pet_members").update({ status: "revoked" }).eq("id", memberId);
    void fetchMembers();
  }
    ...
    <div style={{ fontFamily: FONT }}>
      ...
      {loading ? (
        ...
        <div style={{ padding: "4px 20px 8px" }}>
          {members.map((m) => {
            const displayName = m.user_name ?? m.user_email ?? m.member_id;
            const isOwner = m.member_id === user?.id;
            ...
            return (
              <div key={m.id} style={{
                ...
                <div style={{
                  ...
                  background: m.status === "active" ? `${ACCENT}22` : "rgba(255,159,10,0.15)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                }}>
                  {m.status === "pending"
                    ? <Clock size={18} color="#ff9f0a" />
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: "#fff", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {displayName}
                    {isOwner && <span style={{ marginLeft: 6, fontSize: 11, color: "rgba(235,235,245,0.4)" }}>(tú)</span>}
                  </p>
                  {m.business_name && (
                    <p style={{ margin: "2px 0 0", fontSize: 11, color: "rgba(235,235,245,0.4)" }}>
                      via {m.business_name}
                    </p>
                  )}
                  {m.status === "pending" && (
                    <p style={{ margin: "2px 0 0", fontSize: 11, color: "#ff9f0a", fontWeight: 600 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {m.status === "pending" && !isOwner && (
                    <button
                      ...
                      style={{ width: 32, height: 32, borderRadius: 9, background: "rgba(52,199,89,0.15)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
                      title="Aprobar"
                    >
                  ...
                  {m.status === "active" && (
                    <button
                      onClick={() => setEditingPermsMemberId(m.id)}
                      style={{ width: 32, height: 32, borderRadius: 9, background: "rgba(255,255,255,0.06)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
                  ...
                  {!isOwner && (
                    <button
                      ...
                      style={{ width: 32, height: 32, borderRadius: 9, background: "rgba(255,69,58,0.1)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
